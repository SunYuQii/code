...
parser.add_argument("--gpu_id",
                    default=2,
                    help="the gpu id")
parser.add_argument("--num2match",
                    default=12,
                    type=int,
                    help="the number of pseudo item")
args = parser.parse_args()

dataset = args.dataset
num2match = args.num2match
gpu_id = args.gpu_id
...
# error_list = json.load(open(f"data/{dataset}/pseudo/error_list.json", "r"))
...
for user in tqdm(llmResRaw.keys()):
    ...
    # sentence = "Hanes Mens EcoSmart Fleece Sweatpant\nLeg Avenue Women_s Nylon Striped Tights  Black/Purple  One Size\nExtra Pair Value Eyes Plastic Frames 6 Pack - Incredible Value\nNIKE Women's Flex Supreme TR 4 Cross Trainer"
...
# json.dump(result, open(f"data/{dataset}/pseudo/humanFix.json", "w"))
json.dump(user2pseu,open(f"data/{dataset}/pseudo/raw_list.json","w"))